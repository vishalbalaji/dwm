  ...
  char *windowname;
	...
	if ((w = m->ww - tw - stw - x) > bh) {
      ...
      drw_text(drw, x, 0, w - 2 * sp, bh, mid + lrpad / 2, windowname, 0);
			...
			if (m->sel->issticky)
        drw_rect(drw, x + boxs + mid - 5, (bh / 2) - (boxw / 2) + 0.75, boxw, boxw, 0, 0);

			if (m->sel->isfloating) {
				drw_rect(drw, x + boxs + mid - 5, boxs, boxw, boxw, m->sel->isfixed, 0);
				if (m->sel->isalwaysontop)
					drw_rect(drw, x + boxs + mid - 5, bh - boxw, boxw, boxw, 0, 0);
			}
		} else {
...
drawbars(void)
{
	...
	if (showsystray && !systraypinning && !systrayonleft)
		updatesystray(0);
}
